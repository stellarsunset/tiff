...
/**
 * Copied out of the JDK internals, useful for testing various library components without requiring a file connection.
 */
final class ByteArrayChannel implements SeekableByteChannel {
    ...
    static ByteArrayChannel empty() {
        return fromByteArray(new byte[0]);
    }

    static ByteArrayChannel fromByteArray(byte[] buf) {
        return new ByteArrayChannel(buf, true);
    }

    static ByteArrayChannel fromShortArray(short[] buf) {
        ByteBuffer buffer = ByteBuffer.allocate(buf.length * 2);
        for (int i = 0; i < buf.length; i++) {
            buffer.putShort(i * 2, buf[i]);
        }
        return fromByteArray(buffer.array());
    }

    static ByteArrayChannel fromIntArray(int[] buf) {
        ByteBuffer buffer = ByteBuffer.allocate(buf.length * 4);
        for (int i = 0; i < buf.length; i++) {
            buffer.putInt(i * 4, buf[i]);
        }
        return fromByteArray(buffer.array());
    }

    static ByteArrayChannel fromLongArray(long[] buf) {
        ByteBuffer buffer = ByteBuffer.allocate(buf.length * 8);
        for (int i = 0; i < buf.length; i++) {
            buffer.putLong(i * 8, buf[i]);
        }
        return fromByteArray(buffer.array());
    }
